...
const STORAGE_KEY = 'ghoul-kp-style-v1-short';
...
function scoredEntries(){ return sequence().map((id,i)=>({id,answer:state.answers[i]})).filter(x=>x.answer&&!isCustom(x.answer)&&x.answer.reason); }
...
function renderQuiz(){
  ...
  const choices=(items,kind)=>items.map(item=>`<label class="choice ${answer[kind]===item.id?'selected':''}"><input type="radio" name="${kind}" value="${item.id}" ${answer[kind]===item.id?'checked':''}><span class="choice-key">${item.id}</span><span class="choice-text">${escapeHtml(item.text)}</span></label>`).join('');
  const custom=`<label class="choice custom-choice ${isCustom(answer)?'selected':''}"><input type="radio" name="ruling" value="custom" ${isCustom(answer)?'checked':''}><span class="choice-key">D</span><span class="choice-text"><strong>別の裁定を考えた</strong><small>三択へ無理に合わせず、自分の案を残します</small></span></label>`;
...
function appendReplacement(){
  if(state.mode!=='quick'||!isCustom(state.answers[state.index])||state.answers[state.index].replacementAdded||state.replacementsUsed>=3)return false;
...
function strictnessLabel(value){ return value>=8?['成立探索型','提案や救済が成立する条件を探しやすい。']:value<=-8?['境界維持型','既定条件・失敗・因果の境界を維持しやすい。']:['条件調整型','条件・代償・折衷案を置きやすい。']; }
...
function resultImageCanvas(){
  ...
  c.fillStyle='#b9c4bb';c.font='26px "Yu Gothic UI", sans-serif';c.fillText(`G ${result.scores.G}   H ${result.scores.H}   O ${result.scores.O}   U ${result.scores.U}   L ${result.scores.L}`,80,428);c.fillStyle='#e3b968';c.font='700 28px "Yu Gothic UI", sans-serif';c.fillText(`厳格度 ${result.strictness>0?'+':''}${result.strictness}  ·  ${strict[0]}`,80,486);
...
function renderResult(){
  ...
  if(!result){const customs=customCount();app.innerHTML=`<section class="result-wrap empty-result"><div class="eyebrow">Independent Rulings</div><h1 class="result-name">GHOULタイプを算出できませんでした</h1><p class="result-desc">すべての回答が独自裁定だったため、5軸の得点がありません。これは誤りではなく、この三択形式ではあなたの裁定を十分に表せなかったという結果です。</p><div class="result-metrics"><div><b>0</b><span>採点した回答</span></div><div><b>${customs}</b><span>独自裁定</span></div><div><b>100%</b><span>独自裁定率</span></div></div><div class="quiz-actions"><button class="btn btn-secondary" data-action="restart">診断を選び直す</button><button class="btn btn-primary" data-action="review">回答を見直す</button></div></section>`;return;}
  const type=data.types[result.code], strict=strictnessLabel(result.strictness);lastResult={result,type,strict};
  const valid=scoredEntries().length, customs=customCount(), rate=Math.round(customs/sequence().length*100), reference=(state.mode==='full'&&customs>3)||(state.mode==='quick'&&valid<10);
