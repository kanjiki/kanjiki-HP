...
const ANSWER_HEADERS = ['受信日時','回答日時','回答者名','モード','タイプ','タイプ名','G','H','O','U','L','厳格度','厳格度区分','有効回答数','独自裁定数','回答JSON','設問ID'];
const DETAIL_HEADERS = ['受信日時','回答者名','モード','タイプ','設問ID','回答順','裁定','理由','自由記述'];

function doGet() {
  const template = HtmlService.createTemplateFromFile('index');
  template.appUrl = ScriptApp.getService().getUrl() || '';
  return template.evaluate().setTitle('GHOUL KP STYLE TEST v1.1')
...
/** 初回設定と、既存ブックのv1.1形式への更新を行います。 */
function setupResultsSheet() {
  const properties = PropertiesService.getScriptProperties();
  const existing = properties.getProperty(RESULTS_PROPERTY);
  const book = existing ? SpreadsheetApp.openById(existing) : SpreadsheetApp.create('GHOUL KP STYLE TEST 回答集計 v1.1');
...
function submitResult(payload) {
  ...
  const row = [new Date(), safeCell_(p.submittedAt, 40), safeCell_(p.participantName, 30), safeCell_(p.mode, 20),
    ...
    Number(scores.O || 0), Number(scores.U || 0), Number(scores.L || 0), Number(p.strictness || 0),
...
function prepareAnswersSheet_(book) {
  ...
  sheet.setColumnWidth(16,320); sheet.setColumnWidth(17,210);
  if (sheet.getLastRow() > 1) {
    sheet.getRange(2,1,sheet.getLastRow()-1,1).setNumberFormat('yyyy/mm/dd hh:mm:ss');
...
function prepareSummarySheet_(book) {
  ...
  summary.getRange('A2:A9').setValues([['回答数'],['平均G'],['平均H'],['平均O'],['平均U'],['平均L'],['平均厳格度'],['独自裁定率']]);
  summary.getRange('B2').setFormula("=COUNTA('回答'!A2:A)");
  ['G','H','I','J','K','L'].forEach((column,index) => summary.getRange(3+index,2).setFormula(`=IFERROR(AVERAGE('回答'!${column}2:${column}),0)`));
